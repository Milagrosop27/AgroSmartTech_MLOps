...
const Record = ({ historialAlertas }) => {
  ...
  const alertasSinRespuesta = historialAlertas.filter(a => a.estado === 'TIMEOUT').length;
  const tasaConfirmacion = totalAlertas > 0
    ? Math.round((alertasExitosas / totalAlertas) * 100)
    : 0;

  // Agrupar alertas por hectárea (H1, H2...)
  const alertasPorHectarea = useMemo(() => {
    const mapa = {};
    historialAlertas.forEach(a => {
      const hectarea = a.lote?.split('_')[0] || 'Otro';
      if (!mapa[hectarea]) mapa[hectarea] = { hectarea, total: 0, confirmadas: 0, sinRespuesta: 0 };
      mapa[hectarea].total += 1;
      if (a.estado === 'SUCCESS') mapa[hectarea].confirmadas += 1;
      if (a.estado === 'TIMEOUT') mapa[hectarea].sinRespuesta += 1;
    });
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto w-full">
      ...
      {/* GRÁFICO DE BARRAS — ALERTAS POR HECTÁREA */}
